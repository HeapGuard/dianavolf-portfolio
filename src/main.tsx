...
function Studio() {
  ...
  return <section ref={studio} className="studio" aria-labelledby="studio-title" onPointerMove={moveStudio} onPointerLeave={resetStudio}>
    ...
    <div className="studio__viewport"><div className="studio__window"><img src={voxelStudioRoom} alt="Пиксельная студия: дизайнер работает за компьютером среди растений, полок и кота; в рамке на столе — портрет Дианы Вольф" loading="lazy" decoding="async" fetchPriority="low" /><span className="studio__glass" aria-hidden="true" /><span className="studio__shine studio__shine--one" aria-hidden="true" /><span className="studio__shine studio__shine--two" aria-hidden="true" /></div><p className="studio__caption">смотрим в окно / личная студия</p></div>
  </section>
}
